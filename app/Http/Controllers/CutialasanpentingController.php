...
class CutialasanpentingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $request->file->move(public_path('file/cuti/alasanpenting'), $fileName);
        }
        ...
        return redirect()->route('cutialasanpenting.index')->with('message', 'Data Berhasil Ditambahkan!');
    }

    public function edit(int $id)
    {
        $data = Cutialasanpenting::firstWhere('id', $id);
    ...
    public function update(Request $request, int $id)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            if (is_file(public_path('file/cuti/alasanpenting/').$namaberkas)) {
                unlink(public_path('file/cuti/alasanpenting/').$namaberkas);
            }
            // Upload File Baru
            $fileName = time().'_'.$request->file->getClientOriginalName();
            $request->file->move(public_path('file/cuti/alasanpenting'), $fileName);
        ...
        if ($simpan) {
            return redirect()->route('cutialasanpenting.index')->with('message', 'Data Berhasil Diperbarui!');
        }
    ...
    public function hapus_file(int $id)
    {
        ...
        return redirect()->route('cutialasanpenting.index')->with('message', 'File Berhasil Dihapus!');
    }
    ...
    public function destroy(int $id)
    {
        $data = Cutialasanpenting::firstWhere('id', $id);
        // Hapus Berkas Lama
        unlink(public_path('file/cuti/alasanpenting/').$data->file);
        $data->delete();

        return redirect()->route('cutialasanpenting.index')->with('message', 'Data Berhasil Dihapus!');
    }
}
